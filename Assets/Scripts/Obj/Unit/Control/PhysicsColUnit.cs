...
namespace Obj.Unit.Control
{
    ...
    public class PhysicsColUnit : MonoBehaviour, IInit
    {
        //是否完成初始化
        ...
        //是否是有效的碰撞器
        private bool IsActiveCol(Collider2D col, out PhysicsColUnit otherColUnit)
        {
            PhysicsColUnit unit = col.gameObject.GetComponent<PhysicsColUnit>();
            otherColUnit = unit;

            //有PhysicsColUnit组件+碰撞器位置相同（都是XZ、或都是Y）
            if (null == unit || !(col is BoxCollider2D) || _phyCol.colPos != unit.PhyCol.colPos)
            {
                return false;
            }
            ...
            PhyJudgeType otherType = unit.Phy.judgeType;
