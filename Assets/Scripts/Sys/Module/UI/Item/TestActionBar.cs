...
namespace Sys.Module.UI
{
    ...
    public class TestActionBar : MonoSingleton<TestActionBar>
    {
        ...
        {
            //转变为Hub控制
            HudManager.Instance.isActionHubControl = true;
            if (null != _action)
            {
                //-1是因为开头增加了一个-1
                _objCore.ActionChange_Force(_action, actionFrame.value - 1);
            }
        }
        
        private void OnBtnCancel()
        {
            HudManager.Instance.isActionHubControl = false;
        ...
        private void SetActionFrame(ActionSo action)
        {
            ...
            for (int i = 0; i < action.actionFrame.aniFrameInfos.Count ; i++)
            {
