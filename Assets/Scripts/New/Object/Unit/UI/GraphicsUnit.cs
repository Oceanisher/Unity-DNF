...
namespace New.Object.Unit.UI
{
    ...
    public class GraphicsUnit : AbstractObjUnit
    {
        ...
        private void InitRenderer()
        {
            ...
            if (null == graphics)
            {
                Log.Error($"[GraphicsUnit]Graphics物体缺失。Obj:{gameObject.name}");
                return;
            }
            ...
            foreach (var item in partTypeList)
            {
                ...
                if (item == AniPartType.Body)
                {
                    if (null == body)
                    {
                        Log.Error($"[GraphicsUnit]Body物体缺失。Obj:{gameObject.name}");
                        continue;
                    }
                    ...
                    {
                        Log.Error($"[GraphicsUnit]Body物体精灵渲染器缺失。Obj:{gameObject.name}");
                        continue;
                    }
                ...
                else
                {
                    ...
                    if (null == skin)
                    {
                        Log.Error($"[GraphicsUnit]Body皮肤父节点缺失。Obj:{gameObject.name}");
                        continue;
                    }
                    ...
                    {
                        Log.Error($"[GraphicsUnit]Body皮肤子节点/精灵渲染器缺失。Obj:{gameObject.name}，Part:{item.ToString()}");
                        continue;
                    }
        ...
        private void InitAllSprite()
        {
            ...
            foreach (var pair in _rendererMap)
            {
                ...
                {
                    Log.Error($"[GraphicsUnit]精灵缺失。Type:{pair.Key.ToString()}");
                    continue;
                }
                //获取精灵偏移
                ...
                for (int i = 0 ; i < sprites.Count; i++)
                {
                    ...
                    if (null == sprite)
                    {
                        Log.Error($"[Graphics]缺失精灵。index:{i},type:{pair.Key}");
                        continue;
                    }
        ...
        private Sprite GenerateSprite(Sprite sprite, Vector2Int offset, int ppu)
        {
            Vector2 pivot = 
                new Vector2(
                    0.5f - (offset.x - GameConst.DefaultOffset.x + sprite.rect.width / 2) /
                    sprite.rect.width,
                    0.5f + (offset.y - GameConst.DefaultOffset.y + sprite.rect.height / 2) /
                    sprite.rect.height
                );
